...
vector<vector<int> > Genetic::crossover_2p(vector<int> v1, vector<int> v2) {
	...
	for(int j = 0; j < dim; j++) {
		...
		vector<int> cut_points(2, 0);
		while(cut_points[0] == cut_points[1]) {
			cut_points[0] = rand_in_range(1, var_size - 2);
			cut_points[1] = rand_in_range(1, var_size - 2);
		}
		sort(cut_points.begin(), cut_points.end());
		int left = cut_points[0];
		int right = cut_points[1];
		//Actualizar los hijos
...
vector<int> Genetic::get_schemes_cnt() {
	return schemes_cnt;
}
