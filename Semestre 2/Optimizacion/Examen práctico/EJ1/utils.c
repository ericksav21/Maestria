...
/*----- Functions -----*/
void ri_adj(double *x, double *y, double *p, double *theta, double d4, double *sigma, double *r, int n, int m) {
	for(int i = 0; i < m - 1; i++) {
		r[i] = (theta[i] - atan2(p[0] - x[i], p[1] - y[i])) / sigma[i];
	}
	r[m - 1] = (pow((p[0] - x[m - 1]), 2) + pow((p[1] - y[m - 1]), 2) - d4 * d4) / sigma[m - 1];
}

void J_adj_aprox(double *x, double *y, double *p, double *theta, double d4, double *sigma, double **J, int n, int m, double h) {
	...
	for(int i = 0; i < n; i++) {
		p[i] += h;
		ri_adj(x, y, p, theta, d4, sigma, R2, n, m);
		for(int j = 0; j < m; j++) {
			M[i][j] = R2[j];
		}
		p[i] -= h;
	...
	ri_adj(x, y, p, theta, d4, sigma, R1, n, m);
	for(int i = 0; i < m; i++) {
		for(int j = 0; j < n; j++) {
			J[i][j] = (M[j][i] - R1[i]) / h;
		}
