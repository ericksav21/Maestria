...
int LDLT(double **A, double ***L, double **D, double ***Lt, int n, double tol) {
	for(int i = 0; i < n; i++) {
		...
		double sum1 = 0.0;
		for(int k = 0; k < i; k++) {
			sum1 += ((*L)[i][k] * (*L)[i][k] * (*D)[k]);
		}
		(*D)[i] = A[i][i] - sum1;
		for(int j = i; j < n; j++) {
			(*L)[i][j] = 0.0;
			if((*D)[i] < tol) {
				printf("Error al encontrar la factorización LDLt.\n");
				return 0;
			}
			double sum2 = 0.0;
			for(int k = 0; k < i; k++) {
				sum2 += (*L)[j][k] * (*D)[k] * (*L)[i][k];
			}
			(*L)[j][i] = (*Lt)[i][j] = (A[j][i] - sum2) / (*D)[i];
		}
	}
...
int Jacobi_Iter(double **A, double *b, double *x, int n, int max_iter, int *iter, double tol) {
	...
	for(c = 0; c < max_iter; c++) {
		...
		for(int i = 0; i < n; i++) {
			if(fabs(A[i][i]) < tol) {
				printf("Error al encontrar el vector xt en la iteración %d.\n", c + 1);
				*iter = c + 1;
				free_vector(y);
				return 0;
			}
