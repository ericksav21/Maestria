...
//leerlos todos o no
int read_args(int argc, char **argv, ARGSOBJ *dest) {
	if(argc < 9) {
		...
		printf("[Número de palabras] [Nfeat] [Npop] [Número de iteraciones] ");
		printf("[Métrica de calidad (opcional)]\n");
		printf("[Semilla (opcional)]\n");
		return 0;
	}

	strcpy(dest->fbname, argv[1]);
	strcpy(dest->lbname, argv[3]);
	dest->nff = atoi(argv[2]);
	dest->nfl = atoi(argv[4]);
	dest->nwords = atoi(argv[5]);
	dest->nfeat = atoi(argv[6]);
	dest->npop = atoi(argv[7]);
	dest->niter = atoi(argv[8]);

	if(argc >= 10)
		strcpy(dest->quality_metric, argv[9]);
	if(argc >= 11)
		dest->seed = atoi(argv[10]);
...
void close_items(int *tsc1, int *tsc2, double **fc1, double **fc2, double **frec, double **best_frec, int m1sz, int m2sz, int frec_size) {
	delete_arr1d((void *)tsc1);
	delete_arr1d((void *)tsc2);
	delete_arr2d_d(fc1, m1sz);
	delete_arr2d_d(fc2, m2sz);
	delete_arr2d_d(frec, frec_size);
	delete_arr2d_d(best_frec, frec_size);
}