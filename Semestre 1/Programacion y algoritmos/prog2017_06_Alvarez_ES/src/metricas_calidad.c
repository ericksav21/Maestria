...
FOBJ *mem_obj_alloc(ARGSOBJ *args) {
	...
	n_c_fb = f_separate(args->fbname, args->nff, 1);
	n_c_lb = f_separate(args->lbname, args->nfl, n_c_fb + 1);
	...
	double **best_frec = get_best_frequencies(frec, n_c_lb, no_words, args->nwords);

	//Se generan las matrices de las clases 1 y 2
	printf("\n**---------- Clasificador de texto ----------**\n");
	printf("Generando las clases.\n");
	//Tamaño: n_c_fb
	double **frec_c1 = get_part_of_matrix(best_frec, 0, n_c_fb - 1, args->nwords);
	//Tamaño: n_c_lb
	double **frec_c2 = get_part_of_matrix(best_frec, n_c_fb, n_c_lb - 1, args->nwords);
	...
	FOBJ *params = create_clasifier_params(trying_set_c1, trying_set_c2, frec_c1, frec_c2, m1sz, m2sz, args->nwords);
	//Ya no se necesita más a estos objetos
	close_items(trying_set_c1, trying_set_c2, frec_c1, frec_c2, frec, best_frec, m1sz, m2sz, frec_size);
